...
3. **Pick symbols and colors for each digit** — click a slot button (one per digit: `0 =` and `1 =` for binary, up to `5 =` for senary) to arm it, then click a glyph in the grid below. After each pick the next slot arms automatically, so one grid click per digit completes the setup. The color picker under each slot tints that digit's symbol. Or click **Randomize symbols across all sets** to fill every slot with distinct random symbols drawn from the three non-experimental sets, **Randomize symbols in selected set only** to draw them from the set currently chosen in the next step, and **Randomize colors** to assign each digit a distinct color from a palette that stays readable in both themes.
